...
from app.database.database import get_async_db
from app.models.models import User, UserLevel, Level, Wallet, Task, UserTask, Transaction, TransactionType
from app.routers.auth import get_current_user
from app.schema.schema import BuyLevelRequest, UserLevelResponse, LevelInfoResponse
from app.core.referalservices import process_referral_bonus
...
async def buy_level(
    ...
    # Record transaction for level purchase using enum
    db.add(Transaction(
        user_id=current_user.id,
        type=TransactionType.LEVEL_PURCHASE.value,  # Updated here
        amount=level.earnest_money,
        created_at=datetime.utcnow()
    ))
...
async def upgrade_level(
    ...
    # Record transaction for level upgrade using enum
    db.add(Transaction(
        user_id=current_user.id,
        type=TransactionType.LEVEL_UPGRADE.value,  # Updated here
        amount=difference,
        created_at=datetime.utcnow()
    ))
