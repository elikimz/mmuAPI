...
async def change_user_password(user_id: int, new_password: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_async_db)):
    # Only admin or the user themselves can change
...
# ------------------------
# CHANGE NUMBER (USER OR ADMIN)
# ------------------------
@router.patch("/users/{user_id}/number")
async def change_user_number(user_id: int, new_number: str, country_code: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_async_db)):
    # Only admin or the user themselves can change
    if not getattr(current_user, "is_admin", False) and current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    result = await db.execute(select(User).filter(User.number == full_number, User.id != user_id))
    if result.scalar_one_or_none():
        raise HTTPException(status_code=400, detail=f"The number {full_number} is already used by another user.")
...
# Set PIN (USER)
@router.post("/withdrawal-pin/set")
async def set_withdrawal_pin(request: SetPinRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_async_db)):
    if current_user.withdrawal_pin:
        raise HTTPException(status_code=400, detail="Withdrawal PIN already set. Use change endpoint if needed.")
    current_user.withdrawal_pin = hash_pin(request.pin)
    db.add(current_user)
...
# Change PIN (USER)
@router.post("/withdrawal-pin/change")
async def change_withdrawal_pin(request: SetPinRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_async_db)):
    if not current_user.withdrawal_pin:
        raise HTTPException(status_code=400, detail="No existing PIN found. Please set a PIN first.")
    current_user.withdrawal_pin = hash_pin(request.pin)
    db.add(current_user)
...
# Reset PIN (ADMIN)
@router.post("/withdrawal-pin/reset")
async def admin_reset_withdrawal_pin(request: ResetPinRequest, admin: User = Depends(get_current_admin), db: AsyncSession = Depends(get_async_db)):
    result = await db.execute(select(User).filter(User.id == request.user_id))
    user = result.scalar_one_or_none()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
