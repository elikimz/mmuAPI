...
async def process_task_completion_referral_bonus(
    ...
    # ⭐ VERY IMPORTANT: push pending task completion to DB
    await db.flush()

    # 1️⃣ Check if user still has ANY incomplete tasks
    result = await db.execute(
        select(func.count())
    ...
    # If tasks still remain → stop
    if remaining_tasks > 0:
        return 0

    # 2️⃣ Get A-level referral where bonus NOT yet paid and referrer is active
    result = await db.execute(
        select(Referral)
        .join(User, Referral.referrer_id == User.id)
        .where(
            Referral.referred_id == completed_user_id,
            Referral.level == "A",
            Referral.is_active == False,  # used as bonus_paid flag
            User.is_active == True,
        )
    )
    referral = result.scalar_one_or_none()
